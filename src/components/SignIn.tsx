...
export default function SignIn() {
  ...
  // check for session expired in location state
    ...
    <main className="flex-grow flex flex-col items-center justify-center py-16 px-4 bg-gray-50">
      <div className="w-full max-w-md bg-white p-8 shadow-2xl border border-gray-100">
          ...
          <div className="flex justify-between items-center mt-1">
            <div className="flex items-center gap-2">
              <input 
                ...
                disabled={lockoutTimer > 0 || isLoading || isGoogleLoading}
                className="w-4 h-4 text-brand-maroon focus:ring-brand-maroon border-gray-300 rounded-sm disabled:opacity-50"
              />
              <label htmlFor="keepSignedInPage" className="text-sm text-gray-600 font-medium select-none">
                Keep me signed in
              </label>
            </div>
            <Link to="/forgot-password" className="text-sm font-bold text-brand-maroon hover:underline">
              Forgot Password?
            </Link>
          </div>
        ...
        <p className="mt-8 text-center text-sm text-gray-600 font-medium">
          Don't have an account? <Link to="/register" className="text-brand-maroon font-bold hover:underline">Register</Link>
        </p>
      </div>
    </main>
  );
